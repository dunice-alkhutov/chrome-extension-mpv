...
function sendToNativeApp(data) {
  return new Promise((resolve, reject) => {
    chrome.runtime.sendNativeMessage('com.mpv.app', data, function (response) {
...
function genericOnClick(info, tab) {
  startPlay(info.linkUrl).then(resp => {
    if (resp.code !== 1) {
      console.error('Cant play a video');
    }
  });
}
...
chrome.runtime.onInstalled.addListener(function () {
  ...
  chrome.contextMenus.create(menuItem, _ => {
    console.log('Item was created!', chrome.runtime.lastError);
  });
});
